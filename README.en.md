...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-paper-sculpture-logic)
...
These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 014's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.
...
**Panel 014** is suited to work that must retain the source's essential content while pursuing this visual direction: **Rebuild the photograph as tactile contemporary folded-paper sculpture**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 014 on this image and recommend the best layout and size first.
...
/xxd-panel-014 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-014 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-014 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-014 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 014 to a whole directory; shared settings resolve once
/xxd-panel-014 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
...
Claude Code users may link the same directory to `~/.claude/skills/xxd-panel-014`. Restart the agent session after installation.
...
- [Chinese runtime adapter](references/xxd-panel-014-prompt.zh-CN.md)
- [English runtime adapter](references/xxd-panel-014-prompt.en.md)
...
**This is not a photograph made low-poly; the paper must truly fold out its weight, action, and relation.**
