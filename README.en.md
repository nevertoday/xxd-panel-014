...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 014 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 014's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one paper-sculpture logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 014 contemporary folded-paper sculpture below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 014 contemporary folded-paper sculpture right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `014.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore a different sculpture scale, offset placement, facet rhythm, paper palette, auxiliary pieces, and title relation.
...
## Copy is itself a paper spatial structure
...
Automatic copy distils one short word or title from subject identity, theme, action, emotion, relation, or supported symbolism, adding only a very small number of indexes, places, state words, or micro-notes when useful.

Places, dates, provenance, and factual numbers must be supplied or reliably established and are never invented for sophistication. According to the subject, the main title may arch around the form, extend along an axis, hug a contour, support above or below, interweave through layers, overlap in depth, or suspend in negative space. Copy must still pass the unrelated-image swap test.
...
The image model creates the source-bound paper subject, folded planes, cut edges, layers, nesting, paper palette, tactile material light, and folded-paper type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-014-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-014-prompt.en.md)
- [Original style brief](references/014-source.md)
