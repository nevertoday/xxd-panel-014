---
name: xxd-panel-014
description: "Create XXD Panel 014 artwork from supplied photos in four combinable modes: photo above/contemporary folded-paper sculpture below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Reconstructs source identity, silhouette, pose, and narrative relation through folded planes, cut facets, layered and nested paper pieces, source-derived colour groups, visible fibre and edge thickness, soft natural shadows, source-led off-centre composition, and readable folded-paper typography. Use for the exact 014 handcrafted paper-sculpture editorial style; never use it for cartoon origami, children's craft, smooth plastic 3D, low-poly CG, symmetrical templates, ecommerce mockups, or multi-photo collage."
---

# XXD Panel 014 · 当代折纸雕塑拼贴

Turn each supplied photograph into one finished contemporary folded-paper sculpture and paper-cut collage. Preserve the real photograph only in paired modes; every transformed frame must retain the same source's identity, silhouette, pose, and narrative relation while rebuilding it through folds, facets, layers, nesting, cut edges, and joined paper pieces. Composition follows the source's centre of gravity, contour flow, mass, and action direction rather than a fixed centre, while visible fibre, creases, thickness, and natural shadow keep the result unmistakably handmade.
...
- The transformed frame must preserve source identity through at least three specific cues while using folds, cut facets, layered planes, nesting, overlap, and paper-piece joining to build a single dominant sculpture. Auxiliary paper shapes support balance, direction, depth, or scale and never become a second focal subject. It is not literal photo tracing, cartoon origami, a children's craft, smooth low-poly CG, plastic 3D, symmetrical decoration, ecommerce mockup, or unrelated paper object.
...
Modes and device ratios may change placement and aspect ratio but never the 014 motive. Every transformed frame must visibly express this chain:

**this exact photographed subject or inseparable relation → preserve at least three identity, silhouette, pose, action, structure, scale, negative-shape, or relation cues → read the source's visual centre of gravity, contour flow, mass ratio, and action direction → simplify detail into folded planes, cut facets, nested layers, occlusion, and joined paper pieces → organise one dominant paper sculpture through primary/secondary hierarchy, scale contrast, axis relation, triangular stability, diagonal tension, foreground/background overlap, and positive/negative space → derive a coordinated vivid paper palette from the source using value steps, neighbouring hues, and only a small contrast → keep generous pale negative space and only supportive abstract paper pieces → reveal fibre, crease, cut edge, thickness, and natural shadow under soft diffuse light → make one short readable folded-paper title participate in the sculpture**.

The paper construction comes from the source's actual silhouette, pose, action, mass, direction, opening, overlap, relation, and symbol—not from attaching generic cranes, flowers, geometric bursts, or folded-paper ornaments. Contemporary paper sculpture, paper-cut collage, book arts, tactile editorial still life, and precise publication design are construction influences rather than templates.

Reject the result as generic when an unrelated photo could replace the source without materially changing the sculpture silhouette, fold map, weight distribution, palette, auxiliary supports, or folded title relation. Also reject literal miniature replicas, overly complex stacking, average symmetry, fixed central placement, smooth CGI, shiny plastic, metal, clay, foam board, cheap card, fake bevels, harsh product lighting, cartoon craft, and templated ecommerce display.

## 014 visual system

### Source identity through paper construction
...
- Translate identity into a fold map: which contour becomes a cut edge, which volume becomes a folded plane, which overlap becomes layered paper, which opening becomes negative space, and which relation becomes nesting or occlusion.
...
- Preserve recognition at a glance without recreating every photographic detail. Every facet and layer must clarify identity, action, mass, or relation.

### Source-led editorial composition

- Read the source's visual centre of gravity, contour trajectory, mass ratio, and movement. Place the sculpture accordingly; it may be off-centre, locally cropped, extended toward one edge, or suspended, but never defaults to mechanical centring.
- Use primary/secondary hierarchy, scale contrast, axis relationships, triangular stability, diagonal tension, foreground/background overlap, and positive/negative space to organise one focal sculpture.
- Preserve generous pale negative space as breathing room. Auxiliary paper shards, tabs, arcs, or planes may support weight, direction, or depth but cannot create a competing subject.
- Recompose for each aspect ratio rather than simply centring or cropping the same sculpture. Keep essential folded title, identity cues, and action direction inside safe regions.

### Fold, cut, layer, and nest

- Use folded planes and cut facets to simplify volume; layered, interlocked, nested, and overlapped pieces establish spatial depth and relation.
- Let creases change direction at source-earned joints. Cut edges reveal paper thickness; joints feel physically plausible; planes cast soft contact shadows on neighbouring layers.
- Mix larger structural planes with a limited number of smaller accents. Avoid a confetti of facets, impossible floating pieces, excessive accordion folds, and detail that erases the silhouette.
- Maintain sculptural volume without photoreal replication. It should look built by a skilled paper artist, not exported from a low-poly modeler.

### Source-derived paper palette and material

- Extract the source's most recognisable and vital colours, then convert them into a coordinated, vivid paper group using same-hue value steps, neighbouring hues, and only a small contrasting accent when source-supported.
- Use a pale ivory, light grey, soft pastel, or source-compatible light ground with generous negative space and only restrained paper texture or abstract geometry.
...
## Copy and typography belong to 014

014 text is itself a readable folded-paper structure inside the sculpture—not a detached digital headline, printed sticker, extruded plastic word, or generic caption block.
...
Typography has its own 014 construction:

- **Role:** a spatial paper component that helps brace, wrap, extend, or counterweight the sculpture while remaining clearly readable.
- **Voice:** concise, tactile, poised, contemporary, and source-specific.
- **Construction:** one short main word or title built from folded or layered paper, plus very few small printed or cut-paper notes for balance.
- **Placement:** choose a source-earned relation: arch around the form, extend along an axis, hug a contour, support above or below, interweave through layers, overlap in depth, or suspend in negative space.
- **Material:** folded, cut, layered, or lightly printed matte paper consistent with the sculpture; reveal real crease, edge, thickness, and soft shadow without becoming chunky foam lettering.
- **Script:** use a native paper-construction equivalent for the resolved locale. Preserve natural Chinese, Japanese, Korean, Arabic, and Latin proportions, joining, direction, and legibility rather than forcing Latin block-letter folds onto every script.
...
Prefer GPT Image 2 through the host's built-in bitmap image-generation capability when exposed, and follow the available `imagegen` skill for the built-in execution details. View each local source before generation. Use one generation call per distinct asset; a wallpaper pack requires four calls. “Origami”, “paper sculpture”, “paper-cut collage”, and “folded-paper typography” describe appearance only and never authorise SVG, HTML, CSS, Canvas, diagrams, or programmatic drawing as the artwork.
...
Use only images attached to the current invocation, explicit user paths, or a previously supplied image that the current request clearly identifies as “the same image”. Never broadly scan the Desktop, workspace, `~/Desktop/xxd/xxd-panel-014/`, or historical task folders to find a substitute source. If the intended source cannot be accessed, ask for it.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，014 设计在下）
   2. 左右对照（原图在左，014 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-014-prompt.en.md` or `references/xxd-panel-014-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 014 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 014 transformation on the right.
   - `design-only`: generate the 014 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 014 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 014-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 014-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
`scripts/compose_panel.py` remains available for deterministic recovery and audit. Trigger it only under step 11. When used, generate a same-aspect design asset from the full 014 prompt, preserve the source without destructive crop or stretch, and document which fallback condition applied. A direct complete-canvas success must not be split and recomposed again.
...
Save finished work under `~/Desktop/xxd/xxd-panel-014/` unless the user supplies another destination. Create the shared `~/Desktop/xxd/` wrapper, the skill root, and each task directory when needed.
...
- One dominant paper sculpture preserves at least three source-specific cues and the principal action or inseparable relation; auxiliary pieces never form a second visual centre.
- Folded planes, cut facets, layered and nested paper, overlap, and negative space create plausible hand-built volume without mechanically reproducing photographic detail.
- Composition follows the source's centre of gravity, contour flow, mass ratio, and action direction through hierarchy, scale contrast, axes, triangular stability, diagonal tension, occlusion, and generous pale negative space; it is not mechanically centred or evenly symmetrical.
- A coordinated source-derived paper palette uses same-hue value steps, neighbouring hues, and only a small source-supported contrast on an ivory, light grey, pastel, or compatible pale ground.
- Soft diffuse light reveals paper fibre, crease, cut edge, thickness, layer separation, and natural shadow. Surfaces read as matte paper, not plastic, metal, clay, foam, or smooth CG.
- There is no cartoon origami, children's craft, impossible floating confetti, complex piling, low-poly render, product mockup, ecommerce staging, average symmetry, cheap card, or templated decoration.
- Automatic copy is concise, grounded, and 014-specific; exact user wording remains verbatim. All rendered text is accurate and native to the resolved locale; text-free output contains no text or pseudo-text.
- Typography remains readable folded-paper structure that arches, extends, hugs, supports, interweaves, overlaps, or suspends in a source-earned spatial relation. It never becomes a detached digital title, printed sticker, chunky foam word, generic block lettering, or illegible sculpture.
...
User instructions may change subject emphasis, fold map, facet scale, composition axis, off-centre placement, palette, paper stock, auxiliary shapes, folded-type relation, or copy within 014, but do not silently relax one-photo isolation, exact paired geometry, four separate wallpaper outputs, fresh-task generation, source-hidden output rules, native-language typography, or the hand-built paper-material requirement. Leave the contemporary folded-paper sculpture aesthetic only when the user explicitly asks to leave the 014 style.

## Provenance boundary

The user's original style brief is archived at [references/014-source.md](references/014-source.md). It records the initial 3:4 top-bottom example but does not override the operative mode and explicit final-canvas selection and complete-canvas generation rules. The full local 014 prompt is the production specification. Never borrow subjects, colours, copy, or compositions from samples or previous outputs.
